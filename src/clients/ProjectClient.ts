...
export class ProjectClient extends BaseClient {
  ...
  async getAllProjects(options: {
    ...
    if (options.milestoneId !== undefined) params.append('milestoneId', options.milestoneId.toString());
    if (options.milestoneLabel) params.append('milestoneLabel', options.milestoneLabel);
  ...
  async getProjectClearances(projectId: number, fields?: string): Promise<ProjectClearances> {
    ...
    const response = await this.makeRequest(
      `${this.baseURL}/projects/${projectId}/clearances${params}`
    );
    ...
    return await response.json() as ProjectClearances;
  }
}
