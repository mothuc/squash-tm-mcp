    ...
    try {
        ...
        switch (name) {
            case 'sync_feature_to_squash': {
                const { featureFilePath, testCaseId } = args;
            ...
            case 'create_test_step': {
                const { testCaseId, keyword, action, datatable, comment } = args;
            ...
            case 'update_test_step': {
                const { stepId, keyword, action, datatable, comment } = args;
            ...
            case 'create_keyword_test_case': {
                const { name, parentType, parentId, description, prerequisite, importance, status, automatedTestTechnology } = args;
            ...
            case 'create_bdd_test_case': {
                const { name, parentType, parentId, script, description, prerequisite, importance, status } = args;
            ...
            case 'update_bdd_test_case': {
                const { testCaseId, script, name, description, prerequisite, importance, status } = args;
            ...
            case 'add_step_to_bdd_test_case': {
                const { testCaseId, keyword, text, position } = args;
            ...
            case 'update_step_in_bdd_test_case': {
                const { testCaseId, stepIndex, keyword, text } = args;
            ...
            case 'get_all_projects': {
                const { page, size, type, milestoneId, milestoneLabel } = args;
            ...
            case 'create_project_from_template': {
                const { name, label, description, template_id, params } = args;
...
async function syncFeatureToSquashTM(client, featureFilePath, testCaseId) {
    ...
    const datasetsToDelete = remoteDatasets
        .filter((ds) => !localDatasetNames.has(ds.name))
        .map((ds) => ds.id);
